# Route Manager advance code + some other stuff
# Copyright (c) 2018 Joshua Davidson (it0uchpods)

var max_bank_limit = 25;
var trueSpeedKts = 0;
setprop("/autopilot/internal/max-bank", 15);
setprop("/autopilot/route-manager/advance", 1);
setprop("/autopilot/internal/yaw-damper", 1);
...
var apLoop = maketimer(1, func {
	if (getprop("/autopilot/route-manager/route/num") > 0 and getprop("/autopilot/route-manager/active") == 1) {
		if ((getprop("/autopilot/route-manager/current-wp") + 1) < getprop("/autopilot/route-manager/route/num")) {
			...
			max_bank_limit = getprop("/autopilot/internal/max-bank");
	...
	if (trueSpeedKts > 420) {
		...
		setprop("/autopilot/internal/max-bank", 20);
	} else {
		setprop("/autopilot/internal/max-bank", 25);
	}
});
